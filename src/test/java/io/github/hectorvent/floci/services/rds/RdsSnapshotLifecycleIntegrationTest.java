...
/**
 * The manual snapshot lifecycle against a real Postgres container: a row written before the
 * snapshot comes back from an instance restored from a copy of that snapshot, the source
 * snapshot can be deleted without touching the copy, and each refusal carries the AWS error.
 */
@QuarkusTest
@Tag("docker")
